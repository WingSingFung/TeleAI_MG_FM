...
class SimVQ(nn.Module):
    ...
    def get_codebook_entry(self, indices, shape):
        ...
        z_q = self.embedding_proj(self.embedding(indices))
...
class SimVQ1D(SimVQ):
    def forward(self, z):
        ...
        min_encoding_indices = torch.argmin(d, dim=1).view(z.shape[:2])
        ...
        # compute loss for embedding
        if not self.legacy:
            commit_loss = self.beta * torch.mean((z_q.detach()-z)**2, dim=(1,2)) + \
                   torch.mean((z_q - z.detach()) ** 2, dim=(1,2))
        else:
            commit_loss = (z_q.detach()-z)**2 + self.beta * \
                   (z_q - z.detach()) ** 2
