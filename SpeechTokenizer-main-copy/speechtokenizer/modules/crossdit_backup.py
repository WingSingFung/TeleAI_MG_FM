...
class PreNormWithModulation(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.fn = fn
        self.norm = nn.RMSNorm(dim, eps=1e-4)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.norm_q = nn.RMSNorm(dim, eps=1e-4)
        self.norm_k = nn.RMSNorm(dim, eps=1e-4)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, t = None, context = None, rope=None, sa_mask=None, ca_mask=None):
        ...
        x = self.cross_attn(x, context=context, rope=rope, mask=ca_mask) + x if self.ca else x
...
class DiTDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cond_adapter = nn.Sequential(
            nn.RMSNorm(cond_dim, eps=1e-4),
            nn.Linear(cond_dim, dim),
            Swish(),
            nn.Linear(dim, dim),
            nn.RMSNorm(dim, eps=1e-4)
        )
    ...
    def forward(self, x_t, t, cond):
        ...
        for block in self.layers:
            x_t = block(x_t, t=t_emb, context=cond, rope=self.rope)
...
class DiTEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.post_norm = nn.RMSNorm(dim, eps=1e-4)
